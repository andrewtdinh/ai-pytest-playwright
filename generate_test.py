...
- expect(page).to_have_url() and expect(page).to_have_title() each accept only a string (exact match) or re.Pattern (partial/regex match). NEVER pass a lambda or callable to either — it will raise a TypeError. For partial/contains checks ALWAYS use re.compile() with re.IGNORECASE, e.g. expect(page).to_have_url(re.compile(r"/status_codes/200", re.IGNORECASE)) or expect(page).to_have_title(re.compile(r"Wikipedia", re.IGNORECASE)). Add "import re" after the playwright import when using re.compile().
